...
if uploaded_file:
    ...
    for pipeline_data in pipelines:
        ...
        with col2:
            unique_types = len(
                set(activity["type"] for activity in activities)
            )
            st.metric("Activity Types", unique_types)

        with col3:
            dependency_count = sum(
                len(activity["depends_on"]) for activity in activities
            )
        ...
        st.dataframe(retry_df, width="stretch")
        ...
        # Dependency Graph
        # -----------------------------------
        st.subheader("Dependency Graph Color Guide")
        ...
        dependency_graph = generate_dependency_graph(activities)
        st.graphviz_chart(dependency_graph)

        # -----------------------------------
        # Dataset Lineage
        # -----------------------------------
        st.subheader("Dataset Lineage")
        lineage_graph = generate_lineage_graph(lineage)
